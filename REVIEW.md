...
1. **HTTP Basic Auth with default credentials** (`server/middleware/auth.js:13-14`) — falls back to `admin` / `changeme` if env vars missing. The README says "strongly recommended" — should refuse to start in production without explicit creds.
...
5. **`.env` file present in working tree** with real credentials (`AUTH_USERNAME=hairyone`, `AUTH_PASSWORD=ni1el1`). Not in git history, but make sure this directory isn't synced to any cloud backup (Dropbox/iCloud).

6. **Default `POSTGRES_PASSWORD=password123`** in `.env.example` — fine for local, but the README should be louder about changing it before any non-localhost exposure.
...
7. **No backend tests, no frontend tests** — `client/package.json` no longer has a test script, and `src/` has zero `*.test.*` files. (Previously there was a script but no actual tests; now even the script is gone.)
...
9. **`multer` and `googleapis` were removed** from `package.json` — good. But the `attachments` table still has `google_drive_id` and `google_drive_url` columns; the client just stores a URL. If you're keeping this as a "URL only" model, drop the `google_drive_id` column. If you ever want real Drive integration, you'll need to re-add the SDK and an OAuth flow.
...
- ~~CORS, dead `docker-compose.yml`~~ → Single `docker-compose.yml` for the full stack.
...
7. Add length limits on text fields in DB.

Overall: clean Docker-only stack with a solid foundation. The remaining issues are mostly polish and the test gap.
